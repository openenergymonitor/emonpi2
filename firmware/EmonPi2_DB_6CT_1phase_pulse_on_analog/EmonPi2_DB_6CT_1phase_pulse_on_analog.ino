...
Change Log:
v1.0.0: First release of EmonPi2 DB Continuous Monitoring Firmware (based on EmonTx4 DB Firmware)
...
// Include EmonPi2_DB_6CT_config.ino in the same directory for settings functions & data
...
//---------------------------- emonPi2 Settings - Stored in EEPROM and shared with config.ino ------------------------------------------------
struct {
  ...
  byte nodeID = 27;                                        // node ID for this emonPi2.
...
uint16_t eepromSig = 0x0010;                               // oemEProm signature - see oemEProm Library documentation for details.
...
void loop()             
{
  ...
  if (EmonLibDB_Ready())   
  {
    ...
    for (byte ch=0; ch<NUM_V_CHANNELS; ch++) {
        emonpi.V[ch] = EmonLibDB_getVrms(ch+1) * 100;
    }
    
    for (byte ch=0; ch<NUM_I_CHANNELS; ch++) {
        emonpi.P[ch] = EmonLibDB_getRealPower(ch+1);
    ...
    if (EEProm.json_enabled) {
      ...
      // JSON Format
      // ---------------------------------------------------------------------
      Serial.print(F("{\"MSG\":")); Serial.print(emonpi.Msg);

      for (byte ch=0; ch<NUM_V_CHANNELS; ch++) {
        Serial.print(F(",\"V")); Serial.print(ch+1); Serial.print("\":"); Serial.print(emonpi.V[ch]*0.01);     
      }
      for (byte ch=0; ch<NUM_I_CHANNELS; ch++) {
        Serial.print(F(",\"P")); Serial.print(ch+1); Serial.print("\":"); Serial.print(emonpi.P[ch]);
      }
      for (byte ch=0; ch<NUM_I_CHANNELS; ch++) {
    ...
    // End of print out ----------------------------------------------------
    storeEValues(emonpi.E[0],emonpi.E[1],emonpi.E[2],emonpi.E[3],emonpi.E[4],emonpi.E[5],emonpi.pulse);
  }
  wdt_reset();
  delay(20);
}
